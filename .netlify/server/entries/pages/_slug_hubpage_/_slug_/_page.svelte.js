...
const Page = create_ssr_component(($$result, $$props, $$bindings, slots) => {
  ...
  return `${$$result.head += `<!-- HEAD_svelte-1usmtbr_START -->${$$result.title = `<title>${escape(data.pageTitle)}</title>`, ""}${data.seo ? `${data.seo.description ? `<meta name="${"description"}"${add_attribute("content", data.seo.description, 0)}>` : ``}
    ...
    ${data.seo.description ? `<meta name="${"keywords"}"${add_attribute("content", data.seo.keywords, 0)}>` : ``}` : ``}<!-- HEAD_svelte-1usmtbr_END -->`, ""}
